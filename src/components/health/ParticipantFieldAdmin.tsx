...
type Surface = "list" | "health" | "mail";

const FIELD_TYPES: FieldType[] = ["text", "number", "date", "boolean", "select"];
const SURFACES: Surface[] = ["list", "health", "mail"];
...
// options, event-only surfaces) that doesn't fit ListTemplateAdmin's
...
export default function ParticipantFieldAdmin({ scope, eventId, label }: ParticipantFieldAdminProps) {
  ...
  function openEdit(field: Field) {
    ...
    setSurfaces(new Set(field.surfaces ?? []));
    setFormOpen(true);
  }
            ...
            >
              <div className={active(field) + " flex flex-col"}>
                ...
                <span className="text-[12px] text-ink-secondary">
                  ...
                  {isEvent && field.surfaces && field.surfaces.length > 0
                    ? ` · ${field.surfaces.map((s) => t(`participantFieldAdmin.surface.${s}`)).join(", ")}`
                    : ""}
                </span>
              </div>
              <div className="flex items-center gap-3">
